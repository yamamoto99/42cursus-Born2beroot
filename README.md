...
| 操作 | コマンド | 説明 |
| ------------------------------------ | ------------------------------------------ | -------------------------------------------------------- |
| ユニット（サービス）起動 | `systemctl start ${Unit}` | 指定したユニットを起動します。 |
| ユニット（サービス）停止 | `systemctl stop ${Unit}` | 指定したユニットを停止します。 |
| ユニット（サービス）再起動 | `systemctl restart ${Unit}` | ユニットを停止後、再度起動します。 |
| ユニット（サービス）リロード | `systemctl reload ${Unit}` | ユニットの設定を再読み込みします。 |
| ユニット（サービス）ステータス表示 | `systemctl status ${Unit}` | ユニットの現在の状態を表示します。 |
